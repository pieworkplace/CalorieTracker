...
public class EditExerciseActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DiaryItem exercise = (DiaryItem) getIntent().getSerializableExtra("Exercise");
                ...
                switch (item.getItemId()){
                    ...
                    case R.id.toolbar_edit_done:
                        ...
                        if (titleText.equals("") || calPerUnitText.equals("") || amountText.equals("") || unitNameText.equals("")) {
                            ...
                            DiaryItem exercise = new DiaryItem(titleText, caloryPerUnit, unitNumber, unitNameText);
//                            intent = new Intent();
//                            intent.putExtra("Exercise", exercise);
//                            intent.putExtra("position", position);
//                            setResult(EDIT_RESULT_OK, intent);
                            String dateString = getIntent().getStringExtra("currentDateString");
                            MainActivity.getUserData().getDiary(dateString).editExerciseList(position, exercise);
                            finish();
                        }
                        return true;
                }
...
//                intent.putExtra("position", position);
                String dateString = getIntent().getStringExtra("currentDateString");
                MainActivity.getUserData().getDiary(dateString).removeExerciseList(position);
                finish();
            }
        });
        alertDialog.show();
